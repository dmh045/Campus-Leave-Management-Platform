<template>
  <div class="space-y-6">
    <div v-if="!selectedCourse" class="space-y-6">
      <div
          class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6 flex items-start gap-4"
      >
          ...
          >
            <div @click="handleCourseSelect(course)" class="flex-1 cursor-pointer">
              <div class="flex items-center gap-3 mb-2">
                ...
                >
                  {{ course.status === 'pending' ? '进行中' : course.status === 'completed' ? '已结束' : '未发起' }}
                </span>
              </div>
              ...
              <div class="flex flex-wrap gap-4 text-sm text-gray-600">
                ...
                <div class="flex items-center gap-1">
                  ...
                  应到 {{ course.shouldAttendCount }} / 已到 {{ course.checkedInCount }} / 请假 {{ course.leaveCount }}
                </div>
              </div>
            </div>
...
const dayCourses = ref<TeacherDayCourseDTO[]>([]);
...
// ✅ 修正扫码路由：你项目学生签到页是 /student/checkin
const checkinUrl = computed(() => {
  if (!qrToken.value) return '';
  const base = window.location.origin;
  return `${base}/student/checkin?token=${encodeURIComponent(qrToken.value)}`;
});

// 用 session 叠加状态/人数
const sessionMap = computed(() => {
  const m = new Map<string, AttendanceSessionSummary>();
  for (const s of sessions.value) {
    const k = `${s.offeringId}_${s.courseDate}_${s.sectionStart}_${s.sectionEnd}`;
    m.set(k, s);
  }
  return m;
});

// 今日课程：来自教师当天课表 + session 叠加
const todayCourses = computed(() => {
  const m = sessionMap.value;

  return dayCourses.value.map((c) => {
    const key = `${c.offeringId}_${selectedDate.value}_${c.sectionStart}_${c.sectionEnd}`;
    const s = m.get(key);

    const status = s
        ? (String(s.status || '').toUpperCase() === 'OPEN' ? 'pending' : 'completed')
        : 'not_started';
    ...
    return {
      ...
      period: `第${c.sectionStart}-${c.sectionEnd}节`,
...
const fetchToday = async () => {
  ...
  try {
    const [courses, sess] = await Promise.all([
      // teacher 角色后端可从 token 获取 teacherId，这里仅传 date
      getTeacherDayTimetable(selectedDate.value),
      getTeacherSessions(teacherId.value, selectedDate.value, selectedDate.value),
    ]);

    dayCourses.value = Array.isArray(courses) ? courses : [];
    sessions.value = Array.isArray(sess) ? sess : [];
  } catch (e: any) {
    console.error('fetchToday error:', e);
    ElMessage.error(e?.message || '获取今日课程/场次失败');
    dayCourses.value = [];
    sessions.value = [];
  }
};
...
const handleCourseSelect = async (course: any) => {
  if (!course.sessionId) {
    ElMessage.warning('该课程今天尚未发起签到，请先点击“发起签到”');
    return;
  }
...
const openCheckInModal = async (course: any) => {
  ...
  // 已有 session 则不重复发起
  if (course.sessionId) {
    ElMessage.warning('该课程今天已存在签到场次，请进入详情查看');
    return;
  }
...
watch(selectedCourse, () => {
  ...
  if (!selectedCourse.value) stopPolling();
});
