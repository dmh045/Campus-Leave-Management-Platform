...
public class LeaveController {
    ...
    /**
     * 任课教师端：按课程维度聚合的待确认节次列表
     * 示例：GET /api/leaves/pending/teacher/by-course?teacherId=2
     */
    @GetMapping("/pending/teacher/by-course")
    public Result<List<TeacherCoursePendingDTO>> pendingForTeacherByCourse(@RequestParam Long teacherId) {
        List<TeacherCoursePendingDTO> list = leaveService.listPendingForTeacherByCourse(teacherId);
        return Result.success(list);
    }
    ...
    /**
     * 10.学生重新提交被退回的请假单
     * URL 示例：PUT /api/leaves/1/resubmit
     */
