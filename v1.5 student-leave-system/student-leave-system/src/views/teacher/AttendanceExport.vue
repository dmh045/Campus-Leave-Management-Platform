...
const exportType = ref<'attendance' | 'leave'>('attendance'); // 新增导出类型选择
...
const classes = computed(() => {
  ...
  courses.value.forEach(course => {
    // 如果选择了课程，则只显示该课程对应的班级
    if (selectedCourse.value) {
      if (course.courseId === selectedCourse.value && course.className) {
        classSet.add(course.className);
      }
    } else {
      // 如果没有选择课程，则显示所有班级
      if (course.className) {
...
const getSemesterStartDate = () => {
  ...
  // 如果当前是9-12月，本学期开始于当年9月1日
  if (currentMonth >= 9 && currentMonth <= 12) {
    return `${currentYear}-09-01`;
  } else {
    // 如果当前是1-8月，本学期开始于上一年9月1日
  ...
  try {
    ...
    if (exportType.value === 'attendance') {
      ...
      const fileName = selectedClass.value
          ? `考勤统计表_${selectedClass.value}_${startDate.value}_${endDate.value}.xlsx`
      ...
      // 根据是否选择班级来命名文件
      const fileName = selectedClass.value
          ? `班级请假表_${selectedClass.value}_${startDate.value}_${endDate.value}.xlsx`
          : `班级请假表_${startDate.value}_${endDate.value}.xlsx`;
