...
import {
  getCounselorPendingLeaves,
  counselorApproveLeave,
  counselorBatchApproveLeaves,
  type CounselorPendingLeaveDTO,
  type CounselorAction
} from '@/api/counselor';
...
const mapDtoToUI = (dto: CounselorPendingLeaveDTO): UIRequest => {
  ...
  return {
    ...
    proofUrl: String(dto.proofUrl || ''),  // 确保映射proofUrl字段
    status: 'pending'
  };
};
...
const fetchPending = async () => {
  ...
  try {
    const list = await getCounselorPendingLeaves(counselorId.value);
