  ...
  <div class="space-y-4">
    <div class="bg-white rounded-lg shadow-sm p-4 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <span v-if="selectedCells.length > 0" class="text-sm text-gray-600 font-medium bg-blue-50 px-2 py-1 rounded text-blue-700">
        ...
        <table class="w-full border-collapse table-fixed">
          <thead>
            <tr class="bg-gray-50/80 border-b border-gray-200">
              <th class="w-14 py-3 text-center text-xs font-semibold text-gray-500 border-r border-gray-100">#</th>
              <th class="w-24 py-3 text-center text-xs font-semibold text-gray-500 border-r border-gray-100">时间</th>
              <th v-for="day in weekDayLabels" :key="day" class="py-3 px-2 text-center text-sm font-semibold text-gray-700 border-r border-gray-100 last:border-r-0 min-w-[130px]">
                {{ day }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="sectionNum in 14" :key="sectionNum" class="border-b border-gray-100 last:border-b-0">
              ...
              <td class="h-16 text-center text-sm font-bold text-gray-400 bg-gray-50/30 border-r border-gray-100">
                {{ sectionNum }}
              </td>
                ...
                >
                  <div 
                    ...
                    :class="[
                      ...
                      isSelected(checkCellState(dayIdx, sectionNum).course!) ? 'ring-2 ring-offset-1 ring-blue-500 transform scale-[0.98]' : ''
                    ]"
                  >
                    <div>
                    ...
                    <div>
                      <div class="inline-flex items-center gap-1 bg-white/60 px-2 py-1 rounded-md text-[11px] text-gray-600 font-medium shadow-sm border border-black/5">
...
import LeaveDialog from '@/components/student/LeaveDialog.vue'; // 引入新组件

const currentWeek = ref(1);
const courseList = ref<CourseCell[]>([]);
const selectedCells = ref<string[]>([]); // 存的是 ID
...
const selectedCourseObjects = computed(() => {
  return courseList.value.filter(c => selectedCells.value.includes(c.id));
});
...
const currentTermId = computed(() => {
  ...
  return termId ? Number(termId) : 2;  // 默认为 2
});

// 添加 courseDate 默认值，使用当前日期
...
const checkCellState = (day: number, section: number) => {
  ...
  if (startCourse) {
    return { isStart: true, isOccupied: true, duration: startCourse.duration, course: startCourse };
  }
  const occupiedCourse = courseList.value.find(c => {
    return c.dayOfWeek === day && section > c.section && section < (c.section + c.duration);
  });
  if (occupiedCourse) {
    return { isStart: false, isOccupied: true, duration: 0 };
  }
...
const getLegendColorClass = (key: string) => {
  const map: Record<string, string> = {
    attended: 'bg-green-500', approved: 'bg-yellow-500', pending: 'bg-orange-500',
    public: 'bg-blue-500', absent: 'bg-red-500', cancelled: 'bg-gray-400', rejected: 'bg-purple-500',
  };
  return map[key] || 'bg-gray-300';
};

const statusLabels = {
  attended: '到课', approved: '已请假', pending: '待确认',
  public: '公假', absent: '缺勤/待补假', cancelled: '已取消', rejected: '已驳回',
};
...
const fetchTimetable = async () => {
  // 获取用户信息
  const userId = localStorage.getItem('userId');
  const role = localStorage.getItem('demo_role') || 'unknown';
  ...
  try {
    // 根据角色决定是否传递userId参数
    // 如果用户是学生角色，getStudentTimetable内部会设置为undefined
    // 如果用户不是学生角色，需要显式传递userId
